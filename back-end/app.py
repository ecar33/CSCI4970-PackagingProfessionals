...
def serialize_inventory_changes(log_item):
    """
        @brief Serialize log items to a JSON-friendly dictionary format for API responses.
        
        @param log_item Inventory log model instance to serialize
        
        @return A dictionary containing the id, sku, change type, quantity change, quantity after, timestamp, and note for each inventory log item
    """
    return {
        "id": log_item.id,
        "sku": log_item.sku,
        "change_type": log_item.change_type,
        "quantity_change": log_item.quantity_change,
        "quantity_after": log_item.quantity_after,
...
@app.get("/api/log_inventory")
def get_log_inventory():
    """
        @brief Endpoint to retrieve current inventory log data. This function queries the database for all inventory log items, orders them by sku, and returns a JSON list of serialized inventory items for API responses.
        """
     log_items = db.session.query(InventoryLog).order_by(InventoryLog.sku.asc()).all()
     return jsonify([serialize_inventory_changes(log_item) for log_item in log_items])
